...
## 管理者フロー
1. データ管理：従業員ごとの勤怠情報を詳細画面から独自で編集
...
勤怠データは従業員ユーザーに紐づくため(user_id=1)、
シーディング前に必ず下記のテスト用従業員アカウントを作成してください。
もし既にユーザーがいる場合、idが1になりません
...
本アプリでは、管理者アカウントは初期状態では作成できません。  
管理画面を使用する場合は、管理者用アカウントを作成後、usersテーブルのカラム`role` を DB 上で `admin` に変更してください。
また、管理者アカウントでログインする場合は、確実に従業員側でログアウト処理を行ってから、ログインして下さい
